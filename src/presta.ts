...
const toObject = collection =>
  collection.reduce(
    (acc, cat) => Object.assign({}, acc, { [cat.id]: cat }),
    {}
  );
...
export default class Presta {
  ...
  getResourceList = (collectionName, query) =>
    this.resource(collectionName, collectionName, query) || [];

  getListWithValues = async ({
    collectionName,
    query = {},
    collectionItemName
  }) => {
    const items = await this.getResourceList(collectionName, {
      ...query,
      display: "full"
    });
    ...
    return await Qed({
      ...
      timeout,
      callback: async item =>
        await this.parseResourceItem({
          collectionItemName:
            collectionItemName || this.getCollectionItemName(collectionName),
          item,
          id: item.id
        }),
    });
  ...
  async parseResourceItem({ collectionItemName, item, id }) {
    ...
    const parsedFields = await Promise.all(
      fieldsToParse.map(async key => await model[key](sanitizedEntry[key], id))
    );
    const parsedEntry = parsedFields.reduce(
      (acc, value, index) =>
        Object.assign({}, acc, { [fieldsToParse[index]]: value }),
      sanitizedEntry
    );
    return parsedEntry;
  }
  ...
  async getGenericResourceItem({
    item: { id },
    collectionName,
    collectionItemName: cIN
  }) {
    const collectionItemName =
      cIN || this.getCollectionItemName(collectionName);
    const item = await this.resource(
      collectionItemName,
      `${collectionName}/${id}`
    );
  ...
  _discountPrice = async (price, id) => {
    ...
    const prices = await this.getGenericResource(collectionName, null, {
  ...
  _getLanguageString = data => {
    if (Array.isArray(data)) {
      const item = data.find(
        item => Number(item.id) === this.webservice.options.language
      );
  ...
  _getCachedCollection = (
    ...
    return await Promise.all(
      collection.map(async _item => {
        ...
        const response = await this.getGenericResourceItem(
          Object.assign({}, itemConfig, { item })
        );
        if (cache) {
  ...
  _getAssociations = async (
    { categories, images, product_features, stock_availables },
    id
  ) => {
    ...
    const parsedFeatures = await this._getCachedCollection(
      product_features,
      true
    )({
    ...
    const compoundFeatures = parsedFeatures.map((feature, index) =>
      Object.assign(
        {},
        {
          id_feature_value: parsedFeatureValues[index].id,
          ...parsedFeatureValues[index]
        },
        feature
      )
    );
  ...
  getProduct = id =>
    this.getGenericResourceItem({ item: { id }, collectionName: "products" });
}
